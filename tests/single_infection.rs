...
fn infection_recovery_test() {
    ...
    for attempt in 0..ATTEMPTS {
        ...
        println!(
            "Attempt {} Recover Time: {}",
            attempt,
            recover_time.format("{:d} ({:m} minutes)")
        );
    ...
    assert!(
        avg_time.as_days() >= 3 && avg_time.as_days() < 6,
        "Aiming for default recover time to be between 3 and 6 days, instead {} ({} minutes)",
        avg_time.format("{:d}"),
        avg_time
    );
    println!(
    ...
    let mut variance: f64 = times
        .into_iter()
        .map(|time| ((usize::from(time) as isize - usize::from(&avg_time) as isize) as f64).powi(2))
        .sum();
